    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <form id="myform" method="POST" role="form" action=<?= site_url('Laporan/delete');?> enctype="multipart/form-data" autocomplete="off">
          <input type="hidden" value="Id" name="LaporanId" class="js-input-id" >
...
<script>
$(document).ready(() => {
  $("#LaporanTable").on("click",".js-btn-delete",function(){
    var button=this;
    var tr=$(button.closest("tr")).get();
    let dataLaporan = $(tr[0]).data('laporan');
    $('.js-modal-delete-body').html("Apakah anda yakin menghapus Laporan?");
    $("#deleteLaporan form input[name='LaporanId'").val(dataLaporan.LaporanId);
